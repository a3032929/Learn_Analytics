...
This study will investigate the *Cyber Security Safety at Home, Online, in Life* online course, which is a course delivered by Newcastle University on the FutureLearn platform (https://www.futurelearn.com/courses/cyber-security). There are many factors which could influence the learners' retention rate. Data from activities, such as videos, could act as engagement indicators of the learners and potentially allow early detection of learners' disengagement (Bote-Lorenzo, Gomez-Sanchez, 2017). In this study, we will examine data from the *Cyber Security Safety at Home, Online, in Life* course to understand the factors which influence the learners' retention rate. We will look into if and how the continent the learners come from has an impact on the engagement of learners with the course. This insight will help to ensure that informed decisions can be taken to ensure the course appeals to learners from across the world. 
...
- The CRISP-DM methodology (Cross-Industry Standard Process for Data Mining) will be applied to structure the project life cycle ([*TO DO] link the CRISP-DM guide*). 
...
[CAN THE FIGURE BE REPLACED BY THE FIGURE OF RUN 3 IN SECTION MODELLING?]
...
(IS THIS FOR JOINING ALL RUNS??) 

As the values within the 'viewed_percentage' columns shows the percentage of viewers, we will need to create a new dataset that contains the actual number of viewers (why??). Using a similar function to question 1 and question 2, we will take the total number of learners from the '*Total Viewed*' column, divided by 100, then multiply by the current percentage viewed value from each cell of the 'viewed_percentage' columns. This will calculate the actual number of viewers who have watched 25%/ 50%/ 75% and 100% of the videos. 

The new dataset is then combined with the original '*video_duration*' column and reorganised so that the columns match the original dataset. Values are rounded to full numbers [HOW MANY DECIMAL POINTS INSTEAD OF FULL NUMBERS, e.g values are given as numbers with no decimal points???]. The resulting data frame is stored in the *'cache'* folder. 

The new dataset then undergoes another transformation to calculate the percentage for each of the 'viewed_percentage' columns. The resulting percentage data frame is stored in the *'cache'* 

This dataset will then be combined with the *’video_duration’* column, and then the dataset will be reorganised so that the columns match the original dataset. The columns of the new dataset will be renamed to the corresponding percentage viewed. 

We will use this dataset to plot a line graph to show how the percentage of viewers have changed against the duration of the videos.  

## 7.3 Construct Data (REDUNDANT???)
...
We will take the same data from all of the runs to perform the same investigation. 

The same pre-processing steps will be undertaken as for run 3, to calculate the actual number of viewers from each 'viewed_percentage' columns for each run. The sum of the total actual viewers for each 'viewed_percentage' columns in all runs will be added together, then the percentage of viewers for each 'viewed_percentage' columns will be calculated and stored in *’cache’*. This combines the data from all seven runs together. 
...
Figure XX attempts to demonstrates the relationship between the video duration and the number of views throughout the duration of the videos, at 25%/ 50%/ 75% and 100% of each videos. 
...
The graph shows that there is a steady drop off of learners watching the entire video. Majority of the drop off of learners happen between 75% and 100% way in to the videos.  

There appears to be a tendency of learners to drop off more if the videos are longer too.  
