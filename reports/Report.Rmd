---
title: "CSC8631 Learning Analytics Report"
author: "Selina So"
date: "03/12/2021"
output:
...
The following software sources will be used and are described below under section '1.2.2.1 Requirements':
...
- The CRISP-DM methodology (Cross-Industry Standard Process for Data Mining) will be applied to structure the project life cycle [@RN24]. 
...
There are legal obligations and privacy policies, such as *GDPR (General Data Protection Regulation)* and the *Data Protection Act (2018)*, to consider before using the data (see section '1.2.2.2 Assumptions').
...
- Assumed that although *'Video-stats'* data are not provided for run 1 and 2, this does not indicate that video learning material were not used for these runs. 
...
- There were no descriptions for the data, therefore assumptions will be made as to what the data means. The assumptions are clearly stated throughout the report in the appropriate sections. 

#### Constraints

The project is to be completed by 3rd December 2021. 

Due to the time constraints, the key phases from the CRISP-DM methodology which require focus are *Business Understanding*, *Data Understanding*, *Data Preparation and Modelling* and *Evaluation*. 
...
R was very useful in enabling the analysis of the data. In addition, the packages provided by R allowed the project to be reproducible with minimal effort. The key package is ProjectTemplate, which can automatically build the directory to structure the project and the files, and can automatically load data and libraries.
...
# Data Understanding (Initial Observation)
...
We will make two assumptions for this project: (1) The number of views is equal to the number of learners, i.e. they either do not include the views of tutors or hosts or these are neglible, and (2) each viewer only viewed each video once. 

The data files contain information on the video topics and the absolute number of viewers. This will inform which topics appears to attract the most number of learners. One can also use the range of columns on the 'viewed_percentage', to deduct how the number of learners' engagement changes throughout the duration of the videos. Finally, from the percentage of viewers across the continents, we can observe how engaged the learners from different continents have been throughout the course. Therefore the data file provide a rich source of information to gain insight into the learners behaviour to inform the business objectives of the project. Due to time constraints surrounding this project, the data on the type of devices, downloads, as well as the number of HD users will not be selected for this study. These columns may be interesting for further analysis on how to optimise the content of videos to ensure they are in the right format and have the correct features.

- Basic statistical analysis for run 3 shows that there is some skewness in the *'total_views'* data, ranging from a minimum of 446 total views to 1659 total views. By analysing the number of learners for each topic, it potentially show whether certain topics attract more viewers. 

- Basic statistical analysis for run 3 shows that there is some skewness of the data in columns *'asia_views_percentage'* and *'africa_views_percentage'*, therefore further investigation can potentially show if there are higher drop out rates from some continents compared to others.
...
- From the 'viewed_percent' columns, it is clear from the basic statistical analysis that there is a downward trend in the number of viewers throughout the duration of the videos. Further analysis may help to identify if a maximal duration of the video exist in order to keep learners engaged.
...
### Number of viewers against video topics
...
Figure 1 shows the number of viewers for each video based on the topic of the videos and the absolute number of views for each of these videos.

```{r, echo=FALSE, fig.width=11, fig.height=5.5, fig.cap="Total Number of Views per Video Topic (run 3)"}
...
### Total number of views and features used throughout the videos

A scatterplot matrix will be used to visualize any pairs of relationships. For the plots below the headings, the headings will be on the x-axis, and the corresponding rows will be on the y-axis.

Figure 2 attempts to demonstrate the relationships between the length of the videos and the number of views, captions and transcripts used for each video. It is assumed that the column '*total_transcript_views*' refer to the number of learners reading the transcript version of the videos rather than watching the videos.

```{r, echo=FALSE, fig.width=11, fig.height=7, fig.cap="Total Views and Features used through Video duration (run 3)"}
pairs(~video_duration + total_views + total_caption_views + total_transcript_views, data = run3unite, pch = 19, upper.panel = NULL, cex.main=0.7)
```

There are no obvious relationships observed between the length of the videos and the amount of views and features used for the videos. As the total number of views increases, so does the number of  captions used and transcripts used, which is to be expected. As there are no unexpected patterns that would indicate a dependence of, e.g. the number of transcript views on video duration, we will not continue with investigating these columns in our study.

### Number of viewers for video duration

Figure 3 is a line chart to demonstrate the relationship between the video duration and the percentage of viewers who watched 25%, 50%, 75% and 100% of the video.

```{r, echo=FALSE, fig.width=11, fig.height=7, fig.cap="Number of viewers watched 25%, 50%, 75% and 100% of Videos (run 3)"}
...
The graph shows that there is a steady drop of learners watching the entire video. The majority of the drop out of learners happen between 75% and 100% of the video duration. There is potentially a tendency of learners to drop out more if the videos are longer, however there is too much noise in the data to be certain.

Further investigation will look at this behaviour with data from all runs. This will help understand two potential points: (1) Is there a point throughout the duration of the videos (e.g. between 75% and 100%), independent of total duration of the video, where the drop out rate is the largest? (2) Is there a maximum duration of the video after which the learners decide not to watch the video? This will help answer the question if (1) the content after a certain point in the videos is not of interest for the learners, and (2) how long learners stay engaged for.
...
These values were then combined with the original '*video_duration*' column and re-organised so that the columns match the original data set. Values are given with no decimal places to increase the readability of the results. Since the modelling and evaluation in this project is mainly based on visualizations and qualitative assessments, the accuracy of the data analysis is not affected by this decision. The calculation of the percentages has resulted in the values from the *'antarctica_views_percentage'* being '*NaN*'. Since we know that there were no viewers from Antarctica, all '*NaN*' values have been replaced by 0. The table below shows the relative number of viewers in percentage from each continent for run 3. The data in all remaining runs has been treated in the same way to be included in the second cycle of analysis.  
...
Prior to calculating the percentage of viewers within each continent, the sum of the absolute actual viewers for all runs were added together, thus ensuring that the average is weighted according to the absolute number of learners within each run.

### Format data 

Prior to plotting the data for each cycle, the resulting dataframes are transformed to a 'long' format, i.e., a column for every variable, and a row for every observation (https://www.datacarpentry.org), to enable the use of ggplot2 to plot the data (see Section 3.1.6 *'Modelling'*).

### Modelling 

Figure 4 shows a line chart of the relative number of views as calculated in *Section 3.1.2 Construct data*, against the video duration, i.e. the data that allows to draw comparisons of the behaviour within a continent. This plot shows whether the viewers of a particular continent show an increased or decreased drop out rate compared to the viewers of other continents. As described in *Section 3.1.2 Construct data*, all data is normalised to the most viewed video (*'1.1_Welcome to the course'*, 99 seconds duration).
 
```{r, echo=FALSE, fig.width=11, fig.height=5, fig.cap="Percentage of Viewers against Video duration (run 3)"}
line_cont3_pct_dur_V
```

The percentage of viewers from all continents fluctuate and appears to be independent of the duration of the videos. However it appears as if viewing rates from Africa, Asia and South America show a steeper decline from the *'1.1_Welcome to the course'* video to all other videos in comparison to the other continents. Viewing rates from Oceania show the smallest decline. 

Due to potential noise on the data, the same plot is plotted using the data for all runs (Figure 5). 

```{r, echo=FALSE, fig.width=11, fig.height=5, fig.cap="Percentage of Viewers against Video duration (all runs)"}
line_cont_all_pct_dur_V
```

Figure 5 shows the same pattern as the data of run 3 (Figure 4), in which there is no correlation between the duration of the videos and the number of viewers from each continents. However, it appears as if the steepest decline in viewing numbers can be observed for learners from Africa and Asia. South American viewing rates seemed to show a steeper decline for videos below approx. 220 seconds, whereas they increase compared to the other continents for longer video durations.  
...
## Data Preparation and Modelling for Q2: Does the content of the videos have an impact on the viewing rates across different continents?
...
For the same reasons as explained in Section 3.1.1, the continent columns within the original *'video-stats'* data sets show the percentage of viewers from different continents for each video. To answer question 2, we want to firstly plot a bar chart using the absolute number of viewers from each continent. As explained in Section 3.1.1, we calculated the absolute number of learners by dividing the '*Total Viewed*' column by 100 and multiply by the  percentage viewed value for each continent. For the first cycle of analysis, we will use the data from run 3 only. For the second cycle of analysis, we will combine data from runs 3-7.

### Construct Data

The columns of the new data frame will be renamed to the corresponding continents. This will then be combined with the *'step_title'* column, which as discussed in *'2.3 Explore data'*, consist of combining the two first columns *'step_position'* and *'title'* from the *'video-stats'* data set, to allow quick reference to the order of which the videos appears throughout the course. The data frame will be re-organised so that the columns match the original data set. The table below shows the absolute number of viewers from each continent for run 3. The data in all remaining runs has been treated in the same way to be included in the second cycle of analysis. 
...
### Format data 
Prior to plotting the data for each cycle, the resulting dataframes are transformed to a 'long' format for the plotting of graphs (see Section 3.1.6 *'Modelling'*).

### Modelling 

For the plots in this Section, the video topics will be reordered to the same order as how they run throughout the course. This will show how the number of viewers have changed throughout the course. 

Figure 6 is a bar chart that shows the relationship between the video topics and the number of viewers from across different continents for run 3. The colours in each bar are split according to the continent of the viewers, as shown in the key. 

```{r, echo=FALSE, fig.width=11, fig.height=7, fig.cap="Total continental viewers against Video Topics (run 3)"}
...
The same data, but with the relative number of viewers from each continent, were plotted on a line chart in Figure 7. Each line represents a different continent.

```{r, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Percentage of viewers against Video Topics (run 3)"}
line_cont3_pct_topic_V
```

The line chart shows a steady drop of viewers throughout the course. More learners from Oceania have continued in the course and watched the videos compared to Europe, which was already seen in Section 3.1.6, Figure 4. Asia and Africa show the lowest relative number of viewers. 
...
There is likely to be noise on the data which obscures trends. For the second cycle of analysis, we will plot these graphs again using the data from all of the runs combined. The result is shown in Figure 8.


```{r, echo=FALSE, fig.width=11, fig.height=5, fig.cap="Total continental viewers against Video Topics (all runs)"}
stack_cont_all_AbV_topic_bar_V
```

From the initial observation of the graph, the trend of the viewers from all runs combined, follow the same pattern as for run 3 viewers, with the largest drop in viewers occuring throughout week one of the course. The data shows that the drop in viewers throughout week one is very similar to run 3, from 7458 (at the start of week one) to 3154 (end of week one), which equates to 57.7% in week one alone. Then a smaller gradual decrease of viewers between week two to week three, from 2816 to 1910, which is 32.2%, can be observed. Again, this is a very similar drop as was seen for run 3. 
...
### Modelling 
Figure 10 demonstrates the relationship between the video duration and the number of views throughout the duration of the videos, at 25%, 50%, 75% and 100% of each video, for the data of all runs combined.

```{r, echo=FALSE, fig.width=11, fig.height=7, fig.cap="Number of viewers watched 25%, 50%, 75% and 100% of Videos (all runs)"}
run3_7_allpct_duration_scat_V
```

This graph shows the drop out rate for all runs and it appears to show very similar behaviour of the learners drop out rate as was seen in run 3 (Section 2.3.3, Figure 3). It appears as if three videos have particularly high drop out rates between 75% and 100% of the video duration. However, there does not seem to be any correlation to the video duration. More data would be required to establish whether the trend is real or due to noise. If it is real, it would be interesting to analyse the video content to find potential triggers for the drop outs, or to assess whether this just reflects the lack of content in the last few seconds of the video.
...
*Question 1*: The results appear to indicate that there are no correlations between the length of the videos and how engaged the learners from each continent were. The length of the video does not seem to be a deciding factor for the learners' engagement and retention. 

*Question 2*: The results clearly show that the maximum number of learners will watch the first video of the course, and the number of learners viewing the videos throughout the course drop significantly within the first week. The data suggests that a larger proportion of learners from Africa, Asia, and partially South America drop out compared to the other continents. However, across all continents the drop out of learners between week two and three is not as high as during the first week. It is unclear whether the data provided by FutureLearn also collects the number of learners watching previews of videos, or whether FutureLearn gives free access to their learning material for a short period of time, which could reflect on the sudden drop of learners watching the videos within the first week. The first week block contained videos surrounding the topic of *'Exploring personal privacy online'*. The second week block is on *'Online payment security'* and the third week block is on *'Security in the future home'*.

The line chart in Figures 7 and 9 also indicate that there is a spike in interest for video 2.1 from learners based in South America. The topic was *'Welcome to Week 2: payment security'*. Given more time, we could do further analysis to gain insights about the learners from different continents to understand whether there are other factors which could affect the learners’ interest in certain topics. These factors could include e.g. cultural, economical or political factors of the continent, types of jobs and interest in cyber security within those continents.  

*Question 3*: The results potentially show that the longer a video is, the higher risk of the learners dropping out earlier during the videos. As most learners drop out before they watched 100% of the video, this could help course designers and providers decide whether to ensure that important course content is placed earlier on the videos. Alternatively, it would be interesting to analyse the video contents in the last 25% of the video for the outliers as discussed in Section 3.3.4. This could help understand if there are any triggers for the drop outs to inform the future design of the videos.

It would be interesting to see if the following would have an impact on the data: if the decision is to be made to make important content appear towards the end of the videos, then a contents page could be provided in the video, to inform the learners what to expect throughout a video. This could potentially keep learners more engaged throughout the video. Depending on the interest of the content, it could affect the number of learners watching the entire length of the videos. 

*Summary*: Based on the data and the analysis in this report, it appears that learners engagement with the course decreases throughout the course and throughout the duration of the videos. Most learners appear to stop viewing the videos after the first week of the course. The learners who stay after the first week are more likely to be seriously interested in the course, as they are more likely to stay on for week two and week three. This can be seen by the smaller drop out rate to the course for those two weeks across all continents.  


## Review process 

The outcome of the investigation was satisfactory for the business needs. However, given more time, further analysis could be undertaken on the impact of video features made available for the viewers. With the right quality and quantity of data, it would have been interesting to understand whether learners from different continents prefer to use certain devices to access the video content. This could help inform the future design of course material to ensure it is suitable for all relevant devices. An interesting analysis could also be on whether the captions and transcript features are important for learners who are based in continents and/or countries where English is not the native language of the majority of people. The outcome from this analysis could help optimise the content of videos to ensure that they are in the right format and have the correct features to increase the appeal to learners from across the world. 


In addition, the data could be combined with the data files (e.g. *'enrolments.csv'*) to understand more about the types of learners from each continents, and if the data could be split into separate countries. This could provide further insight to determine if certain topics are of more interest for learners of certain continents and certain countries, which could inform the design of any future courses. For example, we saw that there was potentially more interest for the 'payment security' topics from learners of South America. The enrolment and unenrolment date and time stamps may also give an indication on the average duration that the learners stay on the course. This data may help to confirm that learners are more likely to drop out after the first week of the course.  

As the data was originally collected between 2016 and 2018, the recent pandemic has greatly affected the lives of everyone. Many people were forced to work and study at home, many people lost their jobs, many people had more time to reconsider their career options and many activities have moved online for everyone to access. It will be interesting to analyse and understand what impact has the pandemic had on learners and whether we see more or less engagement from learners viewing the videos.
